...
def create_category(response: Response, category: Category, db: DB, token: Token):
...
def read_categories(response: Response, db: DB, token: Token):
    request_user = auth_tools.verify_token(db, token)
    if request_user is None:
        response.status_code = status.HTTP_403_FORBIDDEN
        return False
...
def read_category_by_id(response: Response, id: str, db: DB, token: Token):
    request_user = auth_tools.verify_token(db, token)
    if request_user is None:
        response.status_code = status.HTTP_403_FORBIDDEN
        return False
...
def update_category(response: Response, id: str, category: Category, db: DB, token: Token):
    request_user = auth_tools.verify_token(db, token)
    if request_user is None:
        response.status_code = status.HTTP_403_FORBIDDEN
        return False
...
def delete_category(response: Response, id: str, db: DB, token: Token):
    request_user = auth_tools.verify_token(db, token)
    if request_user is None:
        response.status_code = status.HTTP_403_FORBIDDEN
        return False
